  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication CJSC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication CJSC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest ineligible-bidder-listed-is-held-and-unoverridable
  (testing "ineligible-bidders-list membership -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:ineligible-bidder-listed} (-> (store/ledger db) last :basis)))
