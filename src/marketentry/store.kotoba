(ns marketentry.store
  "SSoT for the Armenia (ARM) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:on-ineligible-bidders-list?` / `:requires-tin?` / `:tin-verified?`
  are ground truth for the flagship and conditional governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication CJSC" :portal "ARMEPS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-ineligible-bidders-list? false
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ARM" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "ARMEPS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-ineligible-bidders-list? false
             :requires-tin? true :tin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "ARMEPS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ARM" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export CJSC" :portal "ARMEPS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-ineligible-bidders-list? true
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ARM" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "ARMEPS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-ineligible-bidders-list? false
             ...
             :drafted? false :submitted? false
             :jurisdiction "ARM" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "ARMEPS"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? on-ineligible-bidders-list?)   (assoc :engagement/on-ineligible-bidders-list? on-ineligible-bidders-list?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/on-ineligible-bidders-list?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :on-ineligible-bidders-list? (boolean (:engagement/on-ineligible-bidders-list? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-arm"})
